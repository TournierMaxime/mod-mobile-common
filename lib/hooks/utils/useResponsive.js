...
const useResponsive = () => {
  ...
  const imageArticle = () => {
    if (width >= breakpoints.tablet) {
      return [tw`w-full h-99 mt-4`, { objectFit: "cover" }]
    } else {
      return [tw`w-90 h-50 mt-4`, { objectFit: "cover" }]
    }
  }
  ...
  const imageDetails = () => {
    if (width >= breakpoints.tablet) {
      ...
      return [tw`w-60 h-40 rounded-t-md`, { resizeMode: "cover" }]
    }
  }
  ...
  const detailsRole = (text) => {
    if (width >= breakpoints.tablet) {
      return tw`font-medium text-xl px-4 mb-4 text-justify leading-7 ${text}`
    } else {
      return tw`font-medium text-base px-4 mb-4 text-justify leading-7 ${text}`
    }
  }
